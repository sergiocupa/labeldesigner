...
namespace Label.Renderers.Datamatrix
{
    internal class SymbolInfo
    {
        ...
        {
            return lookup(dataCodewords, ESymbolShapeHint.FORCE_NONE, true);
        }


        public static SymbolInfo lookup(int dataCodewords, ESymbolShapeHint shape)
        {
        ...
        {
            ESymbolShapeHint shape = allowRectangular ? ESymbolShapeHint.FORCE_NONE : ESymbolShapeHint.FORCE_SQUARE;
            return lookup(dataCodewords, shape, fail);
        }

        private static SymbolInfo lookup(int dataCodewords, ESymbolShapeHint shape, bool fail)
        {
            return lookup(dataCodewords, shape, null, null, fail);
        }


        public static SymbolInfo lookup(int dataCodewords, ESymbolShapeHint shape, Dimension minSize, Dimension maxSize, bool fail)
        {
            foreach (SymbolInfo symbol in symbols)
            {
                if (shape == ESymbolShapeHint.FORCE_SQUARE && symbol.rectangular)
                {
                    continue;
                }
                if (shape == ESymbolShapeHint.FORCE_RECTANGLE && !symbol.rectangular)
                {
                    continue;
                }
                if (minSize != null
