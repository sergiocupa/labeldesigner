...
namespace LabelDesigner.UI.Fields
{
    public class TextField : DraggableControl
    {
        ...
        public void SetProtocol(LabelProtocol protocol)
        {
            Protocol = protocol;
            Renderer = LabelRenderer.GetInstance(Protocol);
        }

        public LabelProtocol Protocol;

        private GraphicsPath gPath;
        private ILabelRenderer Renderer;
      


        public TextField(LabelProtocol protocol = LabelProtocol.ZPL2)
        {
            Draw      = _Draw;
